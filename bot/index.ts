...
import { Client, GatewayIntentBits, Events, TextChannel, DMChannel, User as DiscordUser } from 'discord.js';
...
class Bot {
    ...
    async respondToUserMention(text: string, channelId: string, authorId: string): Promise<void> {
        if (this.containsAUserRegex.test(text)) {
            ...
            try {
                ...
                let updatedCookie: CookieUser = cookie || {
                    cookies: 0,
                    name,
                    id: userId
                };
